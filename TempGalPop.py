...
class GalPop:
    ...
    def makeField(self, field_name, pk_def, pk_path, pk_sum, pk_folder, cosmo = None, gal_sample = None ):
        ...
        if gal_sample == None: gal_sample = list( range(len(self.IDs)))
        ...
        sp = np.logical_and(np.isin(self.pks[pk_def[0]], g_pks), np.isin(list( range(len(self.IDs))), gal_sample))

        bad_vol = np.sum(pk_sum[:,10][g_pks])
